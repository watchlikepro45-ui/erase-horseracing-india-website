...
export async function GET() {
  try {
    ...
    console.log("Memorials GET - Error:", error)
    console.log("Memorials GET - Data:", data)

    if (error) {
      console.error("Memorials query error:", error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Memorials POST - Error:", error)
      return NextResponse.json({ error: error.message }, { status: 400 })
    }
